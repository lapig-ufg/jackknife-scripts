...
#INPUT_DIR='/data/DADOS_GRID/PVI_BR/pa_br_ndvi_250_lapig'
#OUTPUT_DIR='/data/SENTINEL/BFAST01/output'
OUTPUT_DIR='/data/DADOS_GRID/BFAST10/brazil/'
...
runBfast = function(data, ncores) {
  ...
  bfastPerPixel = function(data) {
    ...
    result = tryCatch({
      data <- na.interp(data)
      ndvi <- ts(data,  frequency = 23, start = 2000 )
      bf1 <- bfast01(ndvi)
      ...
      emptyData <- matrix(ncol = 8, nrow = 1)
      colNames <- c('flag_type','flag_significance','p_segment1','p_segment2','pct_segment1','pct_segment2','flag_pct_stable', 'breakdate')
...
saveBfastResult = function(bfastResult, referenceFile, blockOffset, blockSize) {
  ...
  flagsStableFile = paste0(OUTPUT_DIR, '/', 'flags-stable-', blockOffset[1], '-', blockOffset[2], '.tif')
  saveImage(bfastResult$flag_pct_stable, flagsStableFile, blockOffset, blockSize, referenceFile, 'Float32')
  
  flagsSignificanceFile = paste0(OUTPUT_DIR, '/', 'flags-significance-', blockOffset[1], '-', blockOffset[2], '.tif')
  saveImage(bfastResult$flag_significance, flagsSignificanceFile, blockOffset, blockSize, referenceFile, "Float32")
  ...
  saveImage(bfastResult$breakdate, breakdateFile, blockOffset, blockSize, referenceFile, "Int32")
