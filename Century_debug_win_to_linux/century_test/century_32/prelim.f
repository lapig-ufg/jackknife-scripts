...
c ... Function declarations
      real      catanf, line, tcalc
      external  catanf, line, tcalc

c ... Local variables
      integer   ii, iel, iso, lyr, mm
      real      dely, delx, fcbd(6), fccl(6), fcin(6), fcom(6),
     &          fcsa(6), fcsi(6), fcwp(6), ompc, xslope,
     &          textur, tfunc, agwfunc, bgwfunc, wpbd(6), wpcl(6),
     &          wpin(6), wpom(6), wpsa(6), wpsi(6), wpwp(6), yint
...
c ... Initialize the layers beyond the last one used for safety
      do 18 lyr = nlayer+1, 10
        do 19 iel = 1, 3
          minerl(lyr, iel) = 0.0
19      continue
        asmos(lyr) = 0.0
...
c ....... There is currently no check that only Fallow is chosen
        ...
        agwfunc = 1./(1.+4.*exp(-6.*rwcf(1)))
        bgwfunc = 1./(1.+4.*exp(-6.*rwcf(1)))
